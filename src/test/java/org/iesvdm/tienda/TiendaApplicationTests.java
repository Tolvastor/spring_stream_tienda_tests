...
class TiendaApplicationTests {
    ...
    void test2() {
        ...
        var listPrecios = listProds.stream()
                ...
                .map(pre -> BigDecimal.valueOf(pre).setScale(2, RoundingMode.HALF_UP))
                .map(pre -> pre + "$")
                .toList();
        listPrecios.forEach(System.out::println);
        //TODO
    ...
    void test5() {
        var listFabs = fabRepo.findAll();
        List<Integer> listCod = listFabs.stream() .filter(fabricante -> fabricante.getProductos()!=null && !fabricante.getProductos().isEmpty())
    ...
    void test8() {
        var listFabs = fabRepo.findAll();
        var primeros5 = listFabs.stream()
                .limit(5)
                .toList();
        primeros5.forEach(System.out::println);
    ...
    void test9() {
        var listFabs = fabRepo.findAll();
        var saltar = listFabs.stream()
                .skip(3)
                .limit(2)
                .toList();
        saltar.forEach(System.out::println);
        //TODO
    }
    ...
    void test10() {
        ...
        var masBarato = listProds.stream()
                .sorted(comparing(Producto::getNombre ).thenComparing(Producto::getPrecio))
                .limit(1)
                .toList();
        masBarato.forEach(System.out::println);
        //TODO
    }
    ...
    void test11() {
        var listProds = prodRepo.findAll();
        Optional<Producto> proOpt = listProds.stream()
                .sorted(
                        comparing((Producto producto1) -> producto1.getPrecio(), reverseOrder())
                )
                .findFirst();

        if(proOpt.isPresent()) {
            Producto prod = proOpt.get();
            System.out.println(prod.getNombre() + " " + prod.getPrecio());
        }
        proOpt.ifPresent(x -> System.out.println(x.getNombre() + " " + x.getPrecio()));
        proOpt.ifPresentOrElse(producto -> System.out.println(producto.getNombre() + " " + producto.getPrecio())
    ...
    void test31() {
        var listProds = prodRepo.findAll();
        long numFabsConProds = listProds.stream().map(producto -> producto.getFabricante().getCodigo())
                .distinct()
                .count();
    ...
    void test37() {
        var listProds = prodRepo.findAll();
        var sumaryStatistics = listProds.stream().filter(producto -> producto.getFabricante().getNombre().equals("crucial"))
                .mapToDouble(p -> p.getPrecio())
                .summaryStatistics();
