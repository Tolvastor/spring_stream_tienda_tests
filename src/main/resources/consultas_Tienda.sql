...
select f.codigo from fabricante f where exists (select 1 from producto p where p.fabricante = f.codigo);
...
select nombre from producto where fabricante = 2;
...
select p.nombre, p.precio, f.nombre from producto p join fabricante f on p.fabricante = f.codigo order by f.nombre;

-- 24. Devuelve el nombre del producto, su precio y el nombre de su fabricante, del producto más caro.
select p.nombre, p.precio, f.nombre from producto p join fabricante f on p.fabricante = f.codigo order by p.precio desc limit 1;

-- 25. Devuelve una lista de todos los productos del fabricante Crucial que tengan un precio mayor que 200€.
select p.nombre, p.precio from producto p join fabricante f on p.fabricante = f.codigo where f.nombre = 'Crucial' and p.precio > 200;
-- 26. Devuelve un listado con todos los productos de los fabricantes Asus, Hewlett-Packard y Seagate.
select p.nombre, p.precio, f.nombre from producto p join fabricante f on p.fabricante = f.codigo where f.nombre in ('Asus', 'Hewlett-Packard', 'Seagate');
...
select p.nombre, p.precio, f.nombre from producto p join fabricante f on p.fabricante = f.codigo where p.precio >= 180 order by p.precio desc, p.nombre asc;
...
select f.nombre as fabricante, p.nombre as producto from fabricante f left join producto p on f.codigo = p.fabricante order by f.nombre;
-- 29. Devuelve un listado donde sólo aparezcan aquellos fabricantes que no tienen ningún producto asociado.
select f.nombre as fabricante from fabricante f left join producto p on f.codigo = p.fabricante where p.codigo is null order by f.nombre;
-- 30. Calcula el número total de productos que hay en la tabla productos. Utiliza la api de stream.
select count(*) as total_productos from producto;
-- 31. Calcula el número de fabricantes con productos, utilizando un stream de Productos.
select count(distinct fabricante) as total_fabricantes_con_productos from producto;
...
select count(*) as total_productos from producto where fabricante = (select codigo from fabricante where nombre = 'Asus');
-- 36. Calcula la media del precio de todos los productos del fabricante Asus.
select avg(precio) as precio_medio from producto where fabricante = (select codigo from fabricante where nombre = 'Asus');
...
select max(precio) as precio_maximo, min(precio) as precio_minimo, avg(precio) as precio_medio, count(*) as total_productos from producto where fabricante = (select codigo from fabricante where nombre = 'Crucial');
...
select f.nombre from fabricante f join producto p on f.codigo = p.fabricante group by f.nombre having count(p.codigo) >= 2;
...
select f.nombre from fabricante f join producto p on f.codigo = p.fabricante group by f.nombre having sum(p.precio) > 1000 order by sum(p.precio);
